...
# --------------------------------------------------------------------------
# PLANTILLA DE COMANDO — confirmada contra el --help real de plot_radar.py:
#   --live            -> trae el frame real más reciente de la API de OHMC
#   --radar RADAR     -> radar_code (ej RMA2)
#   --product PRODUCT -> product_key (ej DBZHo, VRAD, COLMAX)
#   --out OUT         -> ruta COMPLETA del PNG de salida (no una carpeta)
# {out_path} lo arma Python con el mismo naming RADAR_PRODUCTO_FECHA_HORA.png
# que usa el resto del sistema, así el archivo siempre cae donde se lo busca.
# --------------------------------------------------------------------------
CMD_TEMPLATE = os.environ.get(
    "CMD_TEMPLATE",
    "python plot_radar.py --live --radar RMA1 --product DBZHo --out {out_path} --palette NWS --extra-ring 260 --zoom-km 240"
)
...
def run_one(radar: str, producto: str) -> bool:
    ...
    out_path = OUTPUT_DIR / f"{radar}_{producto}_{ts}.png"
    cmd = CMD_TEMPLATE.format(
        script=SCRIPT_PATH, radar=radar, producto=producto, outdir=str(OUTPUT_DIR), out_path=str(out_path)
    )
    ...
    try:
        ...
        if not out_path.exists():
            log.warning("plot_radar.py terminó OK para %s/%s pero no se encontró el archivo esperado en %s. stdout: %s", radar, producto, out_path, result.stdout[-300:])
            return False
